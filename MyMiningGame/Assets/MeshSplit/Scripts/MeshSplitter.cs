        ...
        private void CreatePointIndicesMap()
        {
            ...
            for (var i = 0; i < meshIndices.Length; i += 3)
            {
                ...
                var gridPos = new Vector3Int(
                    _parameters.SplitAxes.x ? Mathf.RoundToInt(Mathf.Round(triangleCenter.x / _parameters.GridSize) * _parameters.GridSize) : 0,
                    _parameters.SplitAxes.y ? Mathf.RoundToInt(Mathf.Round(triangleCenter.y / _parameters.GridSize) * _parameters.GridSize) : 0,
                    _parameters.SplitAxes.z ? Mathf.RoundToInt(Mathf.Round(triangleCenter.z / _parameters.GridSize) * _parameters.GridSize) : 0
                );
